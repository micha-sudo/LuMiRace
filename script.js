...
const cars = [
  ...
  { id: "blaze", name: "Yellow Racer", price: 50, stockChance: 0.9, accent: "#ffb21c", image: "download.png", description: "Aero boost" },
  { id: "phantom", name: "Red Racer", price: 150, stockChance: 0.8, accent: "#f2383d", image: "download (9).png", description: "Night predator" },
  { id: "emoji", name: "Emoji Car", price: 200, stockChance: 0.7, accent: "#ffd43b", image: "Emoji car.png", description: "Good mood speed" },
  { id: "titan", name: "Galaxy Racer", price: 300, stockChance: 0.55, accent: "#8d5cff", image: "download (1).png", description: "Cosmic elite drift" },
  { id: "duck", name: "Duck Car", price: 600, stockChance: 0.35, accent: "#ffcf33", image: "Duck car.png", description: "Quack-powered racer" },
  { id: "slime", name: "Slime Car", price: 1500, stockChance: 0.15, accent: "#73ff62", image: "Slime Car.png", description: "Radioactive speed" },
  { id: "alien", name: "Alien Car", price: 2500, stockChance: 0.1, accent: "#65e7ff", image: "Alien car.png", description: "Out-of-this-world speed" },
  { id: "admin", name: "Admin Car", adminOnly: true, accent: "#ff65d7", image: "Admin Car.png", description: "Admin only" },
  { id: "sauzen", name: "Sauzen Car", bundleOnly: true, accent: "#ff8b3d", image: "Sauzen Car.png", description: "Taco Bundle car" },
  { id: "mexico", name: "Mexico Car", bundleOnly: true, accent: "#36c96f", image: "Mexico Car.png", description: "Taco Bundle car" },
  { id: "taco", name: "Taco Car", bundleOnly: true, accent: "#f5c542", image: "Taco Car.png", description: "Taco Bundle car" }
];

const SHOP_RESTOCK_MS = 2 * 60 * 1000;
const SHOP_STOCK_KEY = "lumiShopStock";
const SHOP_NEXT_RESTOCK_KEY = "lumiShopNextRestock";
let shopStock = {};
let nextShopRestock = 0;
let shopRestockTimer;
...
const shopRestockTimerDisplay = document.getElementById("shopRestockTimer");
const adminStockCarSelect = document.getElementById("adminStockCarSelect");
const setStockCarBtn = document.getElementById("setStockCarBtn");
...
const joelsLayer = document.getElementById("joels");
...
const startJoelEventBtn = document.getElementById("startJoelEventBtn");
...
const joelMusic = document.getElementById("joelMusic");
const MUSIC_VOLUME = 0.25;
const JOEL_EVENT_MUSIC_VOLUME = 0.65;
...
const globalShopStock = firebaseConnection ? firebaseConnection.database.ref("shopStock") : null;

[lobbyMusic, driveMusic, crashSound, tacoMusic, galaxyMusic, joelMusic].forEach((audio) => {
  audio.volume = MUSIC_VOLUME;
});
joelMusic.volume = JOEL_EVENT_MUSIC_VOLUME;
...
function persistKnownNames(names) {
  const uniqueNames = [...new Set((names || []).map((name) => normalizeName(name)).filter(Boolean))];
  localStorage.setItem("lumiKnownPlayers", JSON.stringify(uniqueNames));
  return uniqueNames;
}
...
function refreshKnownPlayers() {
  ...
  if (adminStockCarSelect) {
    adminStockCarSelect.innerHTML = cars
      .filter((car) => !car.bundleOnly && !car.adminOnly)
...
if (globalEvents) {
  globalEvents.limitToLast(1).on("child_added", (snapshot) => {
    const data = snapshot.val();
    if (data && (data.eventType === "taco" || data.eventType === "galaxy" || data.eventType === "joel")) {
...
function startRandomEvent() {
  ...
  const options = ["taco", "galaxy", "joel"];
  startEvent(options[Math.floor(Math.random() * options.length)]);
}
...
function startEvent(eventType) {
  ...
  const musicMap = { taco: tacoMusic, galaxy: galaxyMusic, joel: joelMusic };
  playAudio(musicMap[gameState.eventType]);
  ...
  gameArea.classList.toggle("joel-event", gameState.eventType === "joel");

  const bannerMap = {
    taco: "🌮 Taco rain! +5 each",
    galaxy: "🌌 Galaxy event! +10 each",
    joel: "Joël rain! +20 per Joël"
  };
  showBanner(bannerMap[gameState.eventType]);
}
...
function endRandomEvent() {
  ...
  joelsLayer.innerHTML = "";
  gameState.tacos = [];
  gameState.stars = [];
  gameState.joels = [];
  stopEventMusic();
  if (gameState.running) playAudio(driveMusic);
  gameArea.classList.remove("taco-event", "galaxy-event", "joel-event");
...
function generateShopStock() {
  return cars.filter((car) => !car.bundleOnly).reduce((stock, car) => {
    stock[car.id] = car.id === "starter" || !car.adminOnly && Math.random() < (car.stockChance || 0);
    return stock;
  }, {});
}

function saveShopStock() {
  localStorage.setItem(SHOP_STOCK_KEY, JSON.stringify(shopStock));
  localStorage.setItem(SHOP_NEXT_RESTOCK_KEY, String(nextShopRestock));
}

function applyShopStock(stock, restockAt, announce = false) {
  shopStock = { ...stock };
  nextShopRestock = Number(restockAt) || Date.now() + SHOP_RESTOCK_MS;
  saveShopStock();
  buildShop();
  updateShopRestockTimer();
  if (announce) showGlobalMessage("Shop restocked");
}

function loadShopStock() {
  try {
    const savedStock = JSON.parse(localStorage.getItem(SHOP_STOCK_KEY) || "null");
    const savedNextRestock = Number(localStorage.getItem(SHOP_NEXT_RESTOCK_KEY));
    if (savedStock && savedNextRestock > Date.now()) {
      shopStock = savedStock;
      nextShopRestock = savedNextRestock;
      return;
    }
  } catch (error) {
    shopStock = {};
  ...
  nextShopRestock = Date.now() + SHOP_RESTOCK_MS;
  saveShopStock();
}

function updateShopRestockTimer() {
  if (!shopRestockTimerDisplay) return;

  const remaining = Math.max(0, nextShopRestock - Date.now());
  const minutes = Math.floor(remaining / 60000);
  const seconds = Math.floor((remaining % 60000) / 1000);
  shopRestockTimerDisplay.textContent = `Restock: ${minutes}:${String(seconds).padStart(2, "0")}`;
}

function restockShop() {
  const nextStock = generateShopStock();
  const nextRestock = Date.now() + SHOP_RESTOCK_MS;

  if (globalShopStock) {
    globalShopStock.set({ stock: nextStock, nextRestock, updatedAt: Date.now() })
      .catch(() => showBanner("Global stock update failed"));
    return;
  }

  applyShopStock(nextStock, nextRestock, true);
}

function startShopRestockTimer() {
  clearInterval(shopRestockTimer);
  updateShopRestockTimer();
  shopRestockTimer = setInterval(() => {
    if (Date.now() >= nextShopRestock) restockShop();
    updateShopRestockTimer();
  }, 1000);
}

function startGlobalShopStock() {
  if (!globalShopStock) return;

  let firstSnapshot = true;
  globalShopStock.on("value", (snapshot) => {
    const data = snapshot.val();
    if (data && data.stock && data.nextRestock) {
      applyShopStock(data.stock, data.nextRestock, !firstSnapshot);
    } else if (!data) {
      globalShopStock.set({ stock: shopStock, nextRestock: nextShopRestock, updatedAt: Date.now() })
        .catch(() => showBanner("Global stock setup failed"));
    }
    firstSnapshot = false;
  }, () => showBanner("Global stock connection failed"));
}

function setCarInGlobalStock() {
  const carId = adminStockCarSelect && adminStockCarSelect.value;
  const car = cars.find((item) => item.id === carId);
  if (!car || car.bundleOnly || car.adminOnly) return;

  shopStock[car.id] = true;
  saveShopStock();

  if (globalShopStock) {
    globalShopStock.set({ stock: shopStock, nextRestock: nextShopRestock, updatedAt: Date.now() })
      .then(() => showBanner(`${car.name} is globally in stock`))
      .catch(() => showBanner("Global stock update failed"));
    return;
  }

  buildShop();
  showBanner(`${car.name} is in stock`);
}

function buildShop() {
  shopList.innerHTML = cars.filter((car) => !car.bundleOnly).map((car) => {
    const owned = storage.ownedCars.includes(car.id);
    const selected = storage.selectedCar === car.id;
    const inStock = Boolean(shopStock[car.id]);
    const label = car.adminOnly && !owned ? "Admin only" : selected ? "Selected" : owned ? "Owned" : inStock ? `Buy ${car.price}` : "Out of stock";
    const className = car.adminOnly && !owned ? "admin-only" : selected ? "selected" : owned ? "owned" : inStock ? "locked" : "out-of-stock";
    const disabled = car.adminOnly && !owned || owned || !inStock ? "disabled" : "";
  ...
  shopList.querySelectorAll(".shop-button").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (!shopStock[carId]) {
        showBanner("This car is out of stock");
        return;
      }
...
function buildBundles() {
  const bundleCars = cars.filter((car) => car.bundleOnly);
  const alreadyOwned = bundleCars.every((car) => storage.ownedCars.includes(car.id));

  bundlesSection.innerHTML = `
    <div class="panel-head">
      <h2>Taco Bundle</h2>
      <span>5000 diamonds</span>
    </div>
    <div class="bundle-card">
      <img src="Taco bundle.png" alt="Taco Bundle">
      <p>Sauzen Car, Mexico Car and Taco Car</p>
      <button id="buyTacoBundleBtn" class="shop-button" type="button" ${alreadyOwned ? "disabled" : ""}>
        ${alreadyOwned ? "Owned" : "Buy"}
      </button>
    </div>
  `;

  const buyButton = document.getElementById("buyTacoBundleBtn");
  if (!buyButton || alreadyOwned) return;

  buyButton.addEventListener("click", () => {
    if (storage.diamonds < 5000) {
      showBanner("Not enough diamonds");
      return;
    }

    storage.diamonds -= 5000;
    storage.ownedCars = [...new Set([
      ...storage.ownedCars,
      ...bundleCars.map((car) => car.id)
    ])];
    saveProgress();
    buildBundles();
    buildShop();
    renderInventory();
    updateHud();
    showBanner("Taco Bundle unlocked");
  });
}
...
function spawnEventItem() {
  const eventType = gameState.eventType;
  const item = document.createElement("div");

  const layerMap = { taco: tacosLayer, galaxy: starsLayer, joel: joelsLayer };
  const classMap = { taco: "event-item taco", galaxy: "event-item star", joel: "event-item joel" };
  const stateKeyMap = { taco: "tacos", galaxy: "stars", joel: "joels" };

  item.className = classMap[eventType];
  item.style.left = `${roadLanes[Math.floor(Math.random() * roadLanes.length)]}%`;
  item.style.top = "-70px";
  layerMap[eventType].appendChild(item);

  gameState[stateKeyMap[eventType]].push({
    y: -70,
    element: item,
...
function updateEntities(deltaMs) {
  ...
  [
    ...
    [gameState.joels, 20, "+20 diamonds"]
  ...
  gameState.joels = gameState.joels.filter((item) => {
    if (item.collected) return false;
    if (item.y > 760) {
      item.element.remove();
      return false;
...
setStockCarBtn?.addEventListener("click", setCarInGlobalStock);
...
startJoelEventBtn.addEventListener("click", () => startGlobalEvent("joel"));
...
processIncomingGift();
loadShopStock();
startShopRestockTimer();
startGlobalShopStock();
