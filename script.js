...
function getKnownPlayers() {
  ...
  const knownPlayers = Object.keys(profiles)
    .map((name) => normalizeName(name))
    .filter(Boolean);

  const merged = [...knownPlayers, ...(JSON.parse(localStorage.getItem("lumiKnownPlayers") || "[]"))]
    .map((name) => normalizeName(name))
    .filter(Boolean)
    .filter((name, index, array) => array.indexOf(name) === index);

  if (storage.player && !merged.includes(storage.player)) {
    merged.push(storage.player);
  }
...
function ensurePlayerProfile(name) {
  ...
  if (!knownPlayers.includes(safeName)) {
    knownPlayers.push(safeName);
    localStorage.setItem("lumiKnownPlayers", JSON.stringify(knownPlayers));
  }
}
