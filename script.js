...
storage.player = normalizeName(localStorage.getItem("lumiPlayer")) || "";
...
function syncKnownPlayers() {
  ...
  const legacyNames = (() => {
    try {
      const raw = JSON.parse(localStorage.getItem("lumiKnownPlayers") || "[]");
  ...
  const namesFromProfiles = Object.keys(profiles)
    .map((name) => normalizeName(name))
    .filter(Boolean);

  const currentPlayer = normalizeName(storage.player);
  const deviceName = normalizeName(getDeviceName());
  const merged = [...namesFromProfiles, ...legacyNames, ...(currentPlayer ? [currentPlayer] : []), ...(deviceName ? [deviceName] : [])]
    .filter((name, index, array) => name && array.indexOf(name) === index)
    .sort((a, b) => a.localeCompare(b));

  return persistKnownNames(merged);
}
...
function giftPlayer(targetName, gift) {
  ...
  updatePlayerProfile(safeName, (profile) => {
    const previousOwned = Array.isArray(profile.ownedCars) ? profile.ownedCars : ["starter"];

    if (gift.type === "car") {
      if (!previousOwned.includes(gift.carId)) {
        profile.ownedCars = [...previousOwned, gift.carId];
      }
  ...
  persistKnownNames([...Object.keys(readProfiles()), safeName, currentUser, getDeviceName()]);

  if (safeName === currentUser) {
    showGiftOverlay(giftStore.message);
  }
}
